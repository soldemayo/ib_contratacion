function LocucionesIE9() {
	...
	this._tagsBloque = {};
	this._tagCargado = undefined;

	this._getSeccion = function(idLocucion) {
		return idLocucion.split("_")[2];
	}
       ...
       $.ajax({
    	   ...
    	   success : function(objetoJson) {
			   $.each(objetoJson, function(){
				   if (this.id.indexOf("seccion") == -1) _this.crearObjetoAudio(this);
				   else {
					   if (tagsBloqueTab[_this._getSeccion(this.id)] == undefined ){
						   tagsBloqueTab[_this._getSeccion(this.id)] = [];   
					   }
					   tagsBloqueTab[_this._getSeccion(this.id)].push( {
						   idClip : this.id,
						   fichero : this.src
					   })
				   }
			   });
			   for (var key in tagsBloqueTab) {
				   	_this.nuevoTag = key;
					_this.crearNuevoTag("", tagsBloqueTab[key]);
					_this.nuevoTag = undefined;
			   }
	...
	this.play = function(idLocucion){
		...
		var $locucion = this.contenedorAudio.find('#' + idLocucion );
		if ($locucion.length != 0 ) $locucion.trigger("play");
		else {
			var reiniciar = true;
			if (this.getAudioAsignado(idLocucion).length == 0 ) {
				if (this.tagCargado != undefined) this.descargarTag(this.tagCargado); 
			    this.cargarTag(this._getSeccion(idLocucion));
				reiniciar = false;
			}
			this.playAudio(idLocucion, true);
		}
	};
	...
	this.playAudio = function(idClip, restart) {
		...
		if( $audioAsignado.prop('readyState') < 4 ) {
			$audioAsignado.one('canplaythrough', function(){$(this).trigger('play');});
			return;
		}
