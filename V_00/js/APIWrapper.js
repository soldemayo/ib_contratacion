...
var API = null;
var findAPITries = 0;
var estado_api = "API no encontrado";
...
function doInitialize()
{
   ...
   if (_Debug == true)
   {
   	  alert("doInitialize(): api encontrada");
   }
   ...
   if (_Debug == true)
   {
   	  alert("doInitialize(): api.LMSInitialize(\"\") == "+result);
   }

   if (result.toString() != "true")
   {
      var err = ErrorHandler("doInitialize");
   }
...
function doTerminate()
{
   ...
   else
   {
      ...
      if (result.toString() != "true")
      {
         var err = ErrorHandler("doTerminate");
      }
...
function doSetValue(name, value)
{
   ...
   else
   {
      ...
      if (result.toString() != "true")
      {
         var err = ErrorHandler("doSetValue");
      }
   }
...
function LMSIsInitialized()
{
   ...
   else
   {
      ...
      var errCode = api.LMSGetLastError().toString();
      if ((errCode == _NotInitialized) || (errCode == _GeneralGetFailure))
      {
         return false;
      }
      else
...
function ErrorHandler(origen)
{
   ...
   {
      // an error was encountered so display the error description
      var errDescription = "Origen: "+origen+"\n";
...
function getAPI()
{
	...
	estado_api = "<font color='#009900'>La comunicación con el API se ha realizado correctamente</font><br/><br/><font size='16'>Encontrado en la ventana lanzadora</font>";
			...
			(typeof(window.opener) != "undefined") )
	{
		...
		estado_api = "<font color='#009900'>La comunicación con el API se ha realizado correctamente</font><br/><br/><font size='16'>Encontrado en el opener</font>";
	}
	if (	(theAPI == null) &&
			...
			(typeof(window.top.opener) != "undefined") )
	{
		...
		estado_api = "<font color='#009900'>La comunicación con el API se ha realizado correctamente</font><br/><br/><font size='16'>Encontrado en 3er nivel</font>";
	}
	// if the API has not been found
	if (theAPI == null)
	{
		// Alert the user that the API Adapter could not be found
		window.status = "API not loaded - Unable to find an API adapter";
		estado_api = "<font color='#CC0000'>API no encontrado</font>";
	}
